...
// country code so we know which temperature unit is customary there.
async function getCountryCodeFromCoords(lat, lon) {
  try {
...
export async function getWeatherByCoords(lat, lon) {
  ...
  const countryCode = await getCountryCodeFromCoords(lat, lon);
  const unit = unitForCountryCode(countryCode);
  return fetchCurrentConditions(lat, lon, unit);
}
